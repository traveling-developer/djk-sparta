...
`instagram-generator/src/main.ts` runs daily at 16:30 UTC (18:30 Berlin time; well after the news-generator's 02:00 UTC run, so its news exist in Sanity). It collects stateless, non-overlapping content windows. **Currently wired into `collectJobs()`, in that order: table-tennis results from yesterday, soccer results from yesterday, table-tennis match announcements two days ahead — nothing else.** The order matters: it drives the staggered publish times (`SCHEDULE_STAGGER_SECONDS`). The other collectors exist but are unused dead code: today's `releaseDate` news from Sanity (`sanityRead.ts`; note the GitHub workflow passes no `SANITY_*` secrets), the league tables (`leagueTable.ts`; table URL = team URL with `/mannschaft/...` replaced by `/tabelle/gesamt`) and the soccer announcements (`getUpcomingSoccerAnnouncements`, dropped deliberately — not wanted any more). Each item is rendered through the typed HTML/CSS string templates in `src/templates/` (1:1 port of the design in `tmp/insta/`, Sparta branding, Google Fonts Oswald + Inter; shared stylesheet in `styles.ts` scales via the `--s` CSS variable, scraped text goes through `esc()` in `html.ts`) via Playwright `page.setContent` → element screenshot (1080×1350 @2x PNG), then uploaded through Zernio's presign flow and published as an Instagram post (`src/zernio.ts`). `--dry-run` writes PNGs to `./out/` instead of posting; `--samples` additionally skips scraping/Sanity; `--draft` uploads to Zernio but creates each post as an unpublished draft (`isDraft`) instead of publishing. There is deliberately **no** date CLI flag: to test outside the real time window, pass a date to a collector directly (each takes one as an optional first argument, e.g. `getYesterdayResults("20.09.2026")`). **Table-tennis** posts both come from `src/scrape/matchResults.ts` and the same schedule table on the mytischtennis team pages (`td(5)` holds the score, so a filled score means played): `getUpcomingAnnouncements` posts the match two days ahead, `getYesterdayResults` the result the day after — both as 9:16 Stories (`placement: "story"`), adults + Jugend. Announcement kicker = `ageClass ?? league` from `TEAM_PAGES`, result kicker = venue + the full `league` (the caption reads "Sieg in der …", where the age class would not fit). Both collectors scrape all five team pages **independently** (10 requests per run) — deliberate: the job is not time-critical and the two windows stay separately testable; they run sequentially, so it is not a burst. A result cell that is non-empty but does not match `\d+:\d+` is logged (a walkover or a scoring decision must not vanish silently); an empty one is the normal "not played yet". Rows without both team names or without our club are skipped, and the dedupe key carries the team label because Mannschaft I and the Jugend share the name "DJK Sparta Noris Nürnberg". mytischtennis rate-limits bursts (HTTP 429, ~15–20 s cooldown), so the scraper sends the `shared/http.ts` headers, retries with a 5 s base backoff, pauses ~2 s between team pages, logs the row count per page and throws when a page yields no schedule rows at all (a rate-limit interstitial or a stale seasonal URL must not look like a match-free day); if *every* team page fails, `scrapeRows` throws too, so an all-down scrape turns the Action red instead of printing "Nothing to post today." A page whose last scheduled game is already in the past logs a stale-link warning — that is the only signal for the Jugend link, which is a Vorrunde staffel with its own ids and expires after 27.11.2026. Every axios call in the generator carries `REQUEST_TIMEOUT_MS` from `shared/http.ts` (axios has no default timeout, and a hanging request would stall the job for hours). The soccer-fixture adapter `src/scrape/soccerMatches.ts` (shared BFV scraper, see below) is still there but no longer collected. Soccer **results** are posted as 9:16 Stories the day after (`src/scrape/soccerResults.ts`, `kind: "result"`): yesterday's played games (Berlin, stateless like the TT results), adults + juniors, decoded from BFV's obfuscated score fonts (see "Shared soccer scraper" below). Both sports share `src/scrape/club.ts` for "is this our team" (`isOurs`) and the venue wording (`venueOf` → Heimspiel/Auswärtsspiel/Vereinsduell) — put club-identity logic there, not in a scraper.
